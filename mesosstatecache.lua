...
local function fetch_and_cache_state_marathon()
    ...
    local appsRes, err = request("localhost", 8081, "/v2/apps?embed=apps.tasks&label=DCOS_SERVICE_NAME")
...
local function fetch_and_cache_state_mesos()
    ...
    -- Expected to run within lock context.
    local mesosRes, err = request("localhost", 5060, "/master/state-summary")
