...
def valid_user_header(valid_jwt_generator):
    ...
    headers with valid JSON Web Token for `requests` module to use"""
...
def invalid_user_header(valid_jwt_generator):
    """This fixture further simplifies JWT handling by providing a ready-to-use
    headers with invalid JSON Web Token for `requests` module to use. This can be
    useful while testing if i.e. access is denied to unknown user to some
    resource."""
