...
local function fetch_and_cache_state_marathon()
    ...
    local appsRes, err = request("127.0.0.1", 8081, "/v2/apps?embed=apps.tasks&label=DCOS_SERVICE_NAME")
...
local function fetch_and_cache_state_mesos()
    ...
    -- Expected to run within lock context.
    local mesosRes, err = request("127.0.0.1", 5060, "/master/state-summary")
